...
def finalize_band_maps(map_signal: NDArray, map_rms: NDArray, pols: str,
                       ...
                       corr_noise_active: bool = False) -> tuple[dict, dict]:
    ...
    Args:
        ...
        compsep_output: The current sky model for this band, written as `map_skymodel`.

    Returns:
        `(detmap_dict, maps_to_file)`. The detector maps are what compsep receives; `maps_to_file`
        is keyed by dataset name for `write_map_chain_to_file`. When
        `mapmaking.common_res_fwhm` is set, `map_observed_sky` and `map_rms` are the smoothed maps
        that compsep actually used, and `map_fwhm_arcmin` records the beam they are at.
    """
    detmap_dict_out = {}
    ...
    if common_res_fwhm:
        ...
        maps_to_file["map_observed_sky"] = sky_out
        maps_to_file["map_rms"] = rms_out
    ...
    # The orbital-dipole and correlated-noise maps are debug output binned straight from the TODs,
    # and stay at the native beam even when the two above are smoothed.
