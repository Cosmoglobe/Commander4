...
def main(params, params_dict):
    ...
    logger.info(f"MPI split performed, hi from worldrank {worldrank}, subcomrank {mycomm.Get_rank()} from color {color} of size {mycomm.Get_size()}.")
    ...
    if color == 0:
        logger.info(f"Worldrank {worldrank}, subrank {mycomm.Get_rank()} going into compsep loop.")
        tod_loop(mycomm, compsep_master, params.niter_gibbs, params)
    elif color == 1:
        logger.info(f"Worldrank {worldrank}, subrank {mycomm.Get_rank()} going into TOD loop.")
        compsep_loop(mycomm, tod_master, cmb_master, params, use_MPI_for_CMB=True)
    elif color == 2:
        logger.info(f"Worldrank {worldrank}, subrank {mycomm.Get_rank()} going into CMB loop.")
