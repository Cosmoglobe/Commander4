...
def single_det_map_accumulator(det_static: DetectorTOD, det_cs_map: np.array, params: bunch) -> tuple[np.array, np.array]:
    ...
    for scan in det_static.scans:
        ...
        maplib.map_accumulator(detmap_rawobs, (scan_map/scan.gain_est).astype(np.float64), inv_var, pix, ntod, npix)
        maplib.map_accumulator(detmap_signal, ((scan_map - scan.n_corr_est)/scan.gain_est - scan.s_orb).astype(np.float64), inv_var, pix, ntod, npix)
        maplib.map_accumulator(detmap_orbdipole, scan.s_orb.astype(np.float64), inv_var, pix, ntod, npix)
        maplib.map_accumulator(detmap_skysub, (scan.sky_subtracted_tod/scan.gain_est).astype(np.float64), inv_var, pix, ntod, npix)
        if params.sample_corr_noise:
            maplib.map_accumulator(detmap_corr_noise, (scan.n_corr_est/scan.gain_est).astype(np.float64), inv_var, pix, ntod, npix)
