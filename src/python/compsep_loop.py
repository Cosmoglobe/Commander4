...
def compsep_loop(comm, tod_master: int, cmb_master: int, use_MPI_for_CMB=True):
    ...
    if master:
        ...
        if not os.path.isdir("../../output/maps_comps/"):
            os.mkdir("../../output/maps_comps/")
        if not os.path.isdir("../../output/maps_sky/"):
            os.mkdir("../../output/maps_sky/")
    ...
    while True:
        ...
        for i_det in range(len(data)):
            ...
            plt.savefig(f"../../output/maps_sky/map_sky_det{i_det}_chain{chain}_iter{iter}.png")
            plt.close()
            hp.mollview(rms_maps[-1], title=f"RMS map, det {i_det}, chain {chain}, iter {iter}")
            plt.savefig(f"../../output/maps_sky/map_rms_det{i_det}_chain{chain}_iter{iter}.png")
            plt.close()
        ...
        for i_det in range(len(data)):
            ...
            plt.savefig(f"../../output/maps_comps/sky_realization_det{i_det}_chain{chain}_iter{iter}.png")
            plt.close()
            hp.mollview(cmb_sky, title=f"CMB realization at {band_freqs[i_det]:.2f}GHz, det {i_det}, chain {chain}, iter {iter}")
            plt.savefig(f"../../output/maps_comps/CMB_realization_det{i_det}_chain{chain}_iter{iter}.png")
            plt.close()
            hp.mollview(dust_sky, title=f"Thermal dust realization at {band_freqs[i_det]:.2f}GHz, det {i_det}, chain {chain}, iter {iter}")
            plt.savefig(f"../../output/maps_comps/Dust_realization_det{i_det}_chain{chain}_iter{iter}.png")
            plt.close()
            ...
            plt.savefig(f"../../output/maps_comps/foreground_subtr_det{i_det}_chain{chain}_iter{iter}.png")
            plt.close()
            detector_maps.append(detector_map)
