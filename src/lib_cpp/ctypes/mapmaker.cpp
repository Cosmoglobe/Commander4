...
void _map_accumulator_IQU_response_T(T *map, const T *tod, const T weight,
    ...
    if(response_I == 1.0 && response_QU == 1.0){
        _map_accumulator_IQU_T<T>(map, tod, weight, pix, psi, scan_len, num_pix);
        return;
    }
    if(response_I == 0.0 && response_QU == 0.0) return;
    ...
    const T weight_QU = weight * response_QU_T;
    if(response_QU == 0.0){
        for(int64_t itod=0; itod<scan_len; itod++){
    ...
    if(response_I == 0.0){
        for(int64_t itod=0; itod<scan_len; itod++){
            const T cos2psi = static_cast<T>(std::cos(2.0 * psi[itod]));
            const T sin2psi = static_cast<T>(std::sin(2.0 * psi[itod]));
            map[pix[itod] +   num_pix] += tod[itod] * weight_QU * cos2psi;
            map[pix[itod] + 2*num_pix] += tod[itod] * weight_QU * sin2psi;
        }
        return;
    }
    for(int64_t itod=0; itod<scan_len; itod++){
        ...
        map[pix[itod] +   num_pix] += tod[itod] * weight_QU * cos2psi;      // Q
        map[pix[itod] + 2*num_pix] += tod[itod] * weight_QU * sin2psi;      // U
    }
}
...
void _map2tod_IQU_response_T(const T *map, T *tod, int64_t *pix, const double *psi,
                             const double response_I, const double response_QU,
                             int64_t scan_len, int64_t num_pix){
    if(response_I == 1.0 && response_QU == 1.0){
        _map2tod_IQU_T<T>(map, tod, pix, psi, scan_len, num_pix);
        return;
    }
    if(response_I == 0.0 && response_QU == 0.0){
        ...
        if(response_QU == 1.0){
            ...
            for(int64_t itod=0; itod<scan_len; itod++){
                ...
                const T polarized = map[pix[itod] + num_pix] * cos2psi
                                  + map[pix[itod] + 2*num_pix] * sin2psi;
                tod[itod] = response_QU_T * polarized;
            }
        }
        return;
    }
    for(int64_t itod=0; itod<scan_len; itod++){
        ...
        const T sin2psi = static_cast<T>(std::sin(2.0 * psi[itod]));
        tod[itod] = response_I_T * map[pix[itod]]
            + response_QU_T * map[pix[itod] + num_pix] * cos2psi
            + response_QU_T * map[pix[itod] + 2*num_pix] * sin2psi;
    }
}
...
void _map_weight_accumulator_IQU_response_T(T *map, const T weight,
    ...
    const T weight_I_QU = weight * response_I_QU;
    const T weight_QU_sq = weight * response_QU_sq;
    if(response_QU == 0.0){
    ...
    if(response_I == 0.0){
        for(int64_t itod=0; itod<scan_len; itod++){
            const T cos2psi = static_cast<T>(std::cos(2.0 * psi[itod]));
            const T sin2psi = static_cast<T>(std::sin(2.0 * psi[itod]));
            map[pix[itod] + 3*num_pix] += weight_QU_sq * cos2psi*cos2psi;
            map[pix[itod] + 4*num_pix] += weight_QU_sq * sin2psi*cos2psi;
            map[pix[itod] + 5*num_pix] += weight_QU_sq * sin2psi*sin2psi;
        }
        return;
    }
    for(int64_t itod=0; itod<scan_len; itod++){
        ...
        map[pix[itod] +   num_pix] += weight_I_QU * cos2psi;              // IQ
        map[pix[itod] + 2*num_pix] += weight_I_QU * sin2psi;              // IU
        map[pix[itod] + 3*num_pix] += weight_QU_sq * cos2psi*cos2psi;     // QQ
        map[pix[itod] + 4*num_pix] += weight_QU_sq * sin2psi*cos2psi;     // QU
        map[pix[itod] + 5*num_pix] += weight_QU_sq * sin2psi*sin2psi;     // UU
    }
}
...
void map2tod_IQU_response_f64(double *map, double *tod, int64_t *pix, double *psi,
                              double response_I, double response_QU,
                              int64_t scan_len, int64_t num_pix){
    _map2tod_IQU_response_T<double>(map, tod, pix, psi, response_I, response_QU,
                                    scan_len, num_pix);
}
...
void map2tod_IQU_response_f32(float *map, float *tod, int64_t *pix, double *psi,
                              double response_I, double response_QU,
                              int64_t scan_len, int64_t num_pix){
    _map2tod_IQU_response_T<float>(map, tod, pix, psi, response_I, response_QU,
                                   scan_len, num_pix);
}
