# MPI collective-communication and SHT benchmarks (not a test suite, despite its old name).
# On OpenMPI, run with
# mpirun -np <number of nodes> --map-by node --bind-to none \
#     python3 aux/mpi_collectives.py 1e9 <number of cores per node>
...
def collective_bench(comm, size):
...
def collective_bench_inplace(comm, size):
    """Tests performance of collective communications of the given size
    on the given communicator. Uses the buffer protocol-based mpi4py
    scheme, i.e. requires array-like objects.
...
def collective_bench_persistent(comm, size):
    """Tests performance of collective communications of the given size
    on the given communicator. Uses the buffer protocol-based mpi4py
    scheme, i.e. requires array-like objects.
...
def collective_bench_simple(comm, size):
    """Tests performance of collective communications of the given size
...
def bench_SHT(comm, nside, max_nthreads):
    ...
    while nthreads <= max_nthreads:
        comm.Barrier()
        t0 = time()
        alm_to_map(alm, nside, lmax, nthreads=nthreads, out=map)
        t0 = time() - t0
...
#    report_status(comm2)
    ...
    collective_bench(comm2, args.message_size)
    collective_bench_simple(comm2, args.message_size)
    collective_bench_inplace(comm2, args.message_size)
    #collective_bench_persistent(comm2, args.message_size)
