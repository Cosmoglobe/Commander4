"""HDF5 output in the ``litebird_sim`` and Planck reader formats.

Each scan is written to its own file with the pointing/TOD layout consumed by the Commander4
LiteBIRD and Planck readers; a per-band ``filelist.txt`` maps scan ids to file paths. Pointing is
Huffman-compressed with a *single per-scan tree* shared across the band's detectors (pix, psi and
flag all decode with the file's ``hufftree``/``huffsymb``). Each detector also carries Planck-style
``scalars``: micro-gain, sigma0, fknee and alpha.
"""
import logging
import os
import h5py
...
def write_scan_file(path: str, pid: int, data_nside: int, fsamp: float, npsi: int, ntod: int,
                    ...
                    det_scalars: dict[str, NDArray], compress: bool = True) -> None:
    ...
    with h5py.File(path, "w") as f:
        ...
        for name in det_names:
            ...
            f[f"{g}/scalars"] = np.asarray(det_scalars[name], dtype=np.float64)
...
def write_filelist(path: str, entries: list[tuple[int, str]]) -> None:
    ...
    with open(path, "w") as f:
        ...
        f.writelines(f'{pid} "{os.path.abspath(filepath)}" 1 0.0 0.0\n'
