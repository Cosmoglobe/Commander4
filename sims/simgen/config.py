...
blocks carry a ``_name`` attribute (the dict key). This mirrors ``commander4.parameters.parse`` so the
...
def as_bunch_recursive(dict_of_dicts: dict, name: str | None = None) -> Bunch:
    ...
    ``commander4.sky.component`` read to identify a component (``comp_params._name``).
    """
    res = Bunch()
    if name is not None:
